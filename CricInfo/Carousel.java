	...
	public String  carouselfeature(Utilities utility){
		...
		for(String user: prodRecmMap.keySet())
		{
			if(user.equals(utility.username()))
			{
				...
				sb.append("<div id='content04'><div class='post04'><h2 class='title meta' style= 'width: 1133px;'>");
				sb.append("<center><h2 style='font-size: 24px;' 'font-color:black;'>RECOMMENDED PRODUCTS</h2></center>");
				...
				*/
				...
				for(String prod : productsList){
					...
					sb.append("<div id='shop_item1'>");
					sb.append("<h3>"+prodObj.getName()+"</h3>");
					sb.append("<strong>"+prodObj.getPrice()+"$</strong><ul>");
					sb.append("<li id='item1'><img src='images/pro/"+prodObj.getImage()+"' alt='' /></li>");
					sb.append("<li><form method='post' action='Cart'>" +
							...
							"<button class ='btn btn-primary btn-sm' type='submit' class='btnbuy' value='Buy Now'>Buy</button></form></li>");
					...
					sb.append("<li><form method='post' 	action='ViewReview'>"+"<input type='hidden' name='name' value='"+prodObj.getName()+"'>"+
							...
							// "<input type='hidden' name='maker' value='"+prodObj.getRetailer()+"'>"+
							"<input type='hidden' name='access' value='"+" "+"'>"+
							"<button class ='btn btn-primary btn-sm' type='submit' value='ViewReview' class='btnreview'>ViewReview</button></form></li>");
